...
const heroContent = {
  ...
  tagline: {
    ...
    mobile: "PPEPCA — Pakistan Petroleum",
  },
  heading: {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-start text-center h-[500px] sm:h-[450px] md:h-[480px] lg:h-[500px] xl:h-[500px] 2xl:h-[600px] pt-[50px] sm:pt-[60px] md:pt-[65px] lg:pt-[70px] xl:pt-[70px] 2xl:pt-[80px] px-4">
        <div className="inline-flex items-center justify-center mb-4 px-2 sm:px-3 md:px-4 py-1 h-[40px] border border-[#16A831] rounded-full bg-[#16A83112] text-[#16A831] text-sm font-medium">
          <p className="text-[12px] font-['Open_Sans'] sm:text-[14px] md:text-[15px] lg:text-[16px] leading-[1.5] text-[#16A831] font-normal">
            <span className="hidden sm:inline">
              {heroContent.tagline.desktop}
            </span>
            <span className="sm:hidden">{heroContent.tagline.mobile}</span>
          </p>
        </div>
        <h1
        ...
        >
          {heroContent.heading.line1} <br /> {heroContent.heading.line2}
        </h1>
        <p
          ...
          {heroContent.buttons.map((btn, index) =>
            btn.variant === "primary" ? (
              ...
              <Link
                key={index}
                href={btn.href}
                className="w-full cursor-pointer sm:w-auto border border-white text-white hover:bg-white hover:text-black font-medium py-3 px-6 rounded transition flex items-center justify-center"
              >
                {btn.label}
              </Link>
            ),
