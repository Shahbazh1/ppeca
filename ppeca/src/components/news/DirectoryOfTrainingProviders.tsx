    ...
    <section className="w-full bg-[#f8fafc]  py-12">
      ...
      <div className="grid grid-cols-1  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {trainingData.map((item, index) => (
          ...
          >
            {/* Title */}
            <p className="text-center text-[14px] font-normal text-[#334155] leading-relaxed px-2">
              {item.title}
            </p>

            {/* Button */}
            <button className="mt-6 bg-[#16a34a] hover:bg-[#16a831] text-[#ffffff] text-sm font-medium px-6 py-2 rounded-sm transition">
              Download
            </button>
          </div>
        ))}
